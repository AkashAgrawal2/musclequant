...
if(EXISTS "${CMAKE_CURRENT_LIST_DIR}/Qt6LanguageServerPrivateDependencies.cmake")
    ...
    _qt_internal_suggest_dependency_debugging(LanguageServerPrivate
        __qt_LanguageServerPrivate_pkg ${CMAKE_FIND_PACKAGE_NAME}_NOT_FOUND_MESSAGE)
endif()
...
if (NOT QT_NO_CREATE_TARGETS AND Qt6LanguageServerPrivate_FOUND)
    ...
    include("${CMAKE_CURRENT_LIST_DIR}/Qt6LanguageServerPrivateAdditionalTargetInfo.cmake"
        OPTIONAL)
    include("${CMAKE_CURRENT_LIST_DIR}/Qt6LanguageServerPrivateExtraProperties.cmake"
        OPTIONAL)
endif()
...
set(__qt_LanguageServerPrivate_always_load_private_module ON)
set(__qt_LanguageServerPrivate_save_QT_NO_PRIVATE_MODULE_WARNING ${QT_NO_PRIVATE_MODULE_WARNING})
set(QT_NO_PRIVATE_MODULE_WARNING ON)
include("${CMAKE_CURRENT_LIST_DIR}/Qt6LanguageServerPrivate-build.cmake" OPTIONAL)
if (Qt6LanguageServerPrivate_FOUND
        AND NOT Qt6LanguageServerPrivatePrivate_FOUND
        AND NOT TRUE
        AND (
            __qt_LanguageServerPrivate_always_load_private_module
    ...
    find_package(Qt6LanguageServerPrivatePrivate "6.9.2" EXACT
        QUIET
        CONFIG
        PATHS
    ...
    if(NOT Qt6LanguageServerPrivatePrivate_FOUND)
        get_property(Qt6LanguageServerPrivatePrivate_warning_shown GLOBAL PROPERTY
            Qt6LanguageServerPrivatePrivate_warning_shown
        )
        if(NOT Qt6LanguageServerPrivatePrivate_warning_shown)
            message(VERBOSE
                "The private module package 'Qt6LanguageServerPrivatePrivate' "
                "could not be found. It possibly needs to be installed separately with your "
                "package manager."
            )
            set_property(GLOBAL PROPERTY
                Qt6LanguageServerPrivatePrivate_warning_shown ON
            )
        endif()
    endif()
endif()
set(QT_NO_PRIVATE_MODULE_WARNING ${__qt_LanguageServerPrivate_save_QT_NO_PRIVATE_MODULE_WARNING})
unset(__qt_LanguageServerPrivate_save_QT_NO_PRIVATE_MODULE_WARNING)
unset(__qt_LanguageServerPrivate_always_load_private_module)
...
if (NOT QT_NO_CREATE_TARGETS AND Qt6LanguageServerPrivate_FOUND)
    ...
    if(NOT Qt6LanguageServerPrivate_DEFINITIONS)
        ...
        set(updated_defs "")
        foreach(def IN LISTS  Qt6LanguageServerPrivate_DEFINITIONS)
        ...
        endforeach()
        set(Qt6LanguageServerPrivate_DEFINITIONS "${updated_defs}")
        unset(updated_defs)
    endif()
    ...
    foreach(_module_dep ${_Qt6LanguageServerPrivate_MODULE_DEPENDENCIES})
        ...
        list(APPEND Qt6LanguageServerPrivate${_private_suffix}_INCLUDE_DIRS
             ${Qt6${_module_dep}_INCLUDE_DIRS})
        list(APPEND Qt6LanguageServerPrivate${_private_suffix}_PRIVATE_INCLUDE_DIRS
             ${Qt6${_module_dep}_PRIVATE_INCLUDE_DIRS})
        if(_private_suffix)
            list(APPEND Qt6LanguageServerPrivate_PRIVATE_INCLUDE_DIRS
                ${Qt6${_module_dep}_PRIVATE_INCLUDE_DIRS})
        endif()
        list(APPEND Qt6LanguageServerPrivate${_private_suffix}_DEFINITIONS
             ${Qt6${_module_dep}_DEFINITIONS})
        list(APPEND Qt6LanguageServerPrivate${_private_suffix}_COMPILE_DEFINITIONS
...
if (TARGET Qt6::LanguageServerPrivate)
    ...
    if(NOT "LanguageServerPrivate" IN_LIST QT_ALL_MODULES_FOUND_VIA_FIND_PACKAGE)
    ...
    if(NOT QT_NO_CREATE_VERSIONLESS_TARGETS)
        if(CMAKE_VERSION VERSION_LESS 3.18 OR QT_USE_OLD_VERSION_LESS_TARGETS)
            ...
            include("${CMAKE_CURRENT_LIST_DIR}/Qt6LanguageServerPrivateVersionlessAliasTargets.cmake")
        endif()
    endif()
else()
    ...
    if(NOT DEFINED Qt6LanguageServerPrivate_NOT_FOUND_MESSAGE)
        set(Qt6LanguageServerPrivate_NOT_FOUND_MESSAGE
            "Target \"Qt6::LanguageServerPrivate\" was not found.")
